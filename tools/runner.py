...
def infer_net(args, config):
    ...
    infer(base_model, test_dataloader, ChamferDisL1, ChamferDisL2, args, config, logger=logger)

def infer(base_model, test_dataloader, ChamferDisL1, ChamferDisL2, args, config, logger = None):
    ...
    np.savez(
        os.path.join(config.EXPERIMENT_DIR, "pointclouds.npz"), 
        input_ptclds=input_ptclds,
        coarse_ptclds=coarse_ptclds,
        dense_ptclds=dense_ptclds,
    )